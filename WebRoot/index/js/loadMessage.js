window.onload = function () {
	...
	function QueryFirstPage(){
		//var data = {"action":"QueryFirstPage"}
		$.ajax({ url:"/ajax/QueryFirstPage",
			type:"get",
			//data: data,
			cache: false,
			success:function(json){
				$("#content .leaveMessageContent > ul > li").css("display","none")
				nowPage = 1
				first_page.click()
				loadMessage(json)
			},
	...
	function QueryPage(pageNum){
		...
		$.ajax({ url:"/ajax/QueryPage",
			...
			error:function(xhr){
				//alert(xhr.responseText+"777777777")
				},
			dataType: "json"})
	}
